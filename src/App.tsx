...
export default function App() {
  ...
  }, [role, teacherCode, studentTeacherCode, studentClassCode]);
    ...
    if (role === "student" && savedName && savedClass && savedClassCode) {
      const restoreStudentClass = async () => {
        try {
          ...
          // Fetch class info and assigned lessons in parallel
          const [cRes, lRes] = await Promise.all([
            fetch(`/api/classes?code=${encodeURIComponent(savedClassCode)}`),
            fetch(`/api/lessons?classCode=${encodeURIComponent(savedClassCode)}`)
          ]);

          if (cRes.ok) {
            const matched = await cRes.json();
            setStudentActiveClass(matched);
            setStudentTeacherCode(matched.teacherCode);
          }
          if (lRes.ok) {
  ...
  const fetchLessons = async () => {
    try {
      if (role === "student") {
        const savedCode = studentClassCode || localStorage.getItem("edu_student_class_code");
        if (savedCode && savedCode !== "CHUNG") {
          const url = `/api/lessons?classCode=${encodeURIComponent(savedCode)}`;
          const res = await fetch(url);
          const data = await res.json();
          setLessons(data);
          return;
        } else if (savedCode === "CHUNG") {
          const url = "/api/lessons?teacherCode=CHUNG";
          const res = await fetch(url);
          const data = await res.json();
          setLessons(data);
          return;
                  ...
                  <form
                    onSubmit={async (e) => {
                      ...
                      try {
                        ...
                        const lRes = await fetch(`/api/lessons?classCode=${encodeURIComponent(codeInput)}`);
                        if (lRes.ok) {
                          const lData = await lRes.json();
                          setLessons(lData);
                        }
