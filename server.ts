...
// Cache mechanisms for super fast responses
...
async function loadClasses() {
  const now = Date.now();
  if (cachedClasses && (now - lastClassesFetchTime < CACHE_TTL_MS)) {
...
async function loadTeachers() {
  const now = Date.now();
  if (cachedTeachers && (now - lastTeachersFetchTime < CACHE_TTL_MS)) {
...
async function loadLessons() {
  const now = Date.now();
  if (cachedLessons && (now - lastLessonsFetchTime < CACHE_TTL_MS)) {
...
async function loadScores() {
  const now = Date.now();
  if (cachedScores && (now - lastScoresFetchTime < CACHE_TTL_MS)) {
    return cachedScores;
  }
...
app.get("/api/lessons", async (req, res) => {
  try {
    ...
    if (classCode) {
      const codeStr = String(classCode).trim().toUpperCase();
      if (codeStr === "CHUNG") {
        const lessons = await loadLessons();
        const publicLessons = lessons.filter((l: any) => !l.teacherCode || l.teacherCode.trim().toUpperCase() === "CHUNG");
        const metadata = publicLessons.map((l: any) => ({
          id: l.id,
          title: l.title,
          subject: l.subject,
          grade: l.grade,
      ...
      const [classes, lessons] = await Promise.all([loadClasses(), loadLessons()]);
      const matchedClass = classes.find((c: any) => c.code && c.code.trim().toUpperCase() === codeStr);
      if (!matchedClass) {
        return res.json([]);
      }

      const assignedIds = matchedClass.assignedLessonIds || [];
      const filteredLessons = lessons.filter((l: any) => assignedIds.includes(l.id));
